...
plt.savefig('./Classification-KNN-SVM/result/homework2/dataset.png')
...
for n in [1,2,3,4]:
    for c in [1e1,1e2,1e3]:
        hyper_plane,b,sv_ind = SVM(X,L,c=c,kernel='polynomial',kargs=[1,0,n])
        plt.figure()
        for i in range(N):
        ...
        plt.title('n = %.1f, c = %.0f'%(n,c))
        plt.savefig('./Classification-KNN-SVM/result/homework2/n_c_%d.png'%(kk))
        ...
        fig.colorbar(surf)
        plt.title('n = %.1f, c = %.0f'%(n,c))
        plt.savefig('./Classification-KNN-SVM/result/homework2/n_c_%d_3D.png'%(kk))
